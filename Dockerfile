...
RUN git clone https://github.com/iotinga/mosquitto-oauth-plugin.git \
    && cd mosquitto-oauth-plugin \
    && git checkout tags/v${OAUTH_PLUGIN_VERSION} \ 
    && mkdir build && cd build \
    && cmake .. \
    && make \
    && make install \
    && cd .. \
    && rm -rf mosquitto-oauth-plugin
